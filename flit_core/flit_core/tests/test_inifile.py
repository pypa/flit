...
def test_missing_entrypoints():
    with pytest.raises(FileNotFoundError):
        inifile.read_pkg_ini(osp.join(samples_dir,'entrypoints_missing.ini'))
...
def test_load_toml():
    inf = inifile.read_pkg_ini(osp.join(samples_dir, 'module1-pkg.toml'))
...
def test_missing_description_file():
    with pytest.raises(inifile.ConfigError, match=r"Description file .* does not exist"):
        inifile.read_pkg_ini(osp.join(samples_dir, 'missing-description-file.toml'))

def test_bad_description_extension(caplog):
    info = inifile.read_pkg_ini(osp.join(samples_dir, 'bad-description-ext.toml'))
...
def test_extras():
    info = inifile.read_pkg_ini(osp.join(samples_dir, 'extras.toml'))
...
def test_extras_dev_conflict():
    with pytest.raises(inifile.ConfigError, match=r'dev-requires'):
        inifile.read_pkg_ini(osp.join(samples_dir, 'extras-dev-conflict.toml'))

def test_extras_dev_warning(caplog):
    info = inifile.read_pkg_ini(osp.join(samples_dir, 'requires-dev.toml'))
...
def test_requires_extra_env_marker():
    info = inifile.read_pkg_ini(osp.join(samples_dir, 'requires-extra-envmark.toml'))
